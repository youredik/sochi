 ...
 *
 * 152-ФЗ инварианты (research-validated 2026-05-29):
 *   - ст.12: Yandex Vision ДОЛЖЕН оставаться RU-region (ocr.api.cloud.yandex.net).
 *     Свап на non-RU OCR-процессор → трансграничная передача (РКН pre-notify +
 *     локализация ст.18 ч.5) → базис ст.6 здесь сломается. Region = инвариант.
 *   - ст.6 ч.3/ч.4: no-consent путь корректен только при действующем поручении
 *     (DPA) с Yandex (категории ПДн + операции + цель + конфиденциальность + ст.19).
 *     Yandex-как-обработчик НЕ требует согласия субъекта, если наша обработка на
 *     базисе ст.6 ч.1 (РКН-практика + b-152 разбор).
 */
