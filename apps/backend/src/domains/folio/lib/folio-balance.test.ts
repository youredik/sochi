...
describe('microsToMinor — boundary + sign', () => {
	...
	test('[M2] round-half-up positive boundaries', () => {
	...
	test('[M3] round-half-up negative (away from zero)', () => {
		expect(microsToMinor(-4_999n)).toBe(0n)
		expect(microsToMinor(-5_000n)).toBe(-1n)
		expect(microsToMinor(-9_999n)).toBe(-1n)
		expect(microsToMinor(-15_000n)).toBe(-2n)
	...
	test('[M4] realistic RUB amount conversion', () => {
	...
	test('[M5] near-Int64-max does not overflow JS BigInt', () => {
...
describe('minorToMicros — exact', () => {
	test('[L1] exact ×10000 (no rounding loss)', () => {
	...
	test('[L2] round-trip: minor → micros → minor preserves value', () => {
	...
	test('[L3] round-trip property (random bigints)', () => {
		void fc.assert(
			fc.property(minorArb, (v) => {
				expect(microsToMinor(minorToMicros(v))).toBe(v)
			}),
		)
	})
...
describe('computeChargesMinor — sub-state filter + sum', () => {
	test('[C1] empty array → 0', () => {
		expect(computeChargesMinor([])).toBe(0n)
	})

	test('[C2] only posted lines summed', () => {
		const lines = [
	...
	test('[C3] draft lines NEVER counted', () => {
		const lines = [
			line({ amountMinor: '500', lineStatus: 'posted' }),
	...
	test('[C4] void lines NEVER counted', () => {
		const lines = [
			line({ amountMinor: '500', lineStatus: 'posted' }),
	...
	test('[C5] negative-amount posted lines (reversals) decrement', () => {
		const lines = [
	...
	test('[C6] sum equals manual reduce of posted-only', () => {
		void fc.assert(
	...
	test('[C7] order-independent (sum is commutative)', () => {
		void fc.assert(
...
describe('computeAccommodationBaseMinor — flag + sub-state filter', () => {
	test('[A1] only posted+isAccommodationBase=true counted', () => {
		const lines = [
	...
	test('[A2] posted+isAccommodationBase=false → not counted', () => {
	...
	test('[A3] draft+isAccommodationBase=true → not counted', () => {
...
describe('computeTourismTaxMinor — НК РФ ст.418.5 + Сочи 2026', () => {
	...
	test('[T1] floor binds when proportional < floor', () => {
	...
	test('[T2] rate binds when proportional > floor', () => {
	...
	test('[T10] property: tax >= floor × nights', () => {
		void fc.assert(
			fc.property(validTaxArgsArb, (args) => {
	...
	test('[T11] property: tax >= base × rateBps / 10000', () => {
		void fc.assert(
			fc.property(validTaxArgsArb, (args) => {
...
describe('computeBalanceMinor — exact arithmetic', () => {
	...
	test('[B2] charges - payments + refunds (exact)', () => {
		expect(
			computeBalanceMinor({
				chargesMinor: 1000n,
	...
	test('[B3] negative balance = overpayment / credit', () => {
	...
	test('[B4] property: equal payment & refund cancel', () => {
		void fc.assert(
			fc.property(balanceArb, ({ chargesMinor }) => {
				const x = 12_345n
...
describe('applyPayment / applyRefund — sign + invariants', () => {
	...
	test('[P3b] zero amount is allowed (boundary: distinguishes < from <=)', () => {
	...
	test('[P4] property: applyPayment(applyRefund(b, x), x) === b', () => {
		void fc.assert(
			fc.property(inverseArb, ([balance, x]) => {
				expect(applyPayment(applyRefund(balance, x), x)).toBe(balance)
			}),
		)
	})

	test('[P4b] property: applyRefund(applyPayment(b, x), x) === b', () => {
		void fc.assert(
			fc.property(inverseArb, ([balance, x]) => {
...
describe('verifyBalanceConservation — drift detector', () => {
	test('[V1] returns null when stored matches computed', () => {
		const lines = [
			line({ amountMinor: '1000', lineStatus: 'posted' }),
			line({ amountMinor: '500', lineStatus: 'posted' }),
		]
	...
	test('[V2] returns diff when stored != computed', () => {
