...
const CODE_TO_RU: Readonly<Record<string, string>> = {
	...
	// 503 от drain-guard при деплое (lib/lifecycle.ts) — НЕ постоянная ошибка,
	// инстанс перезапускается; запрос можно безопасно повторить.
	SERVICE_DRAINING: 'Сервис обновляется — повторите запрос через пару секунд.',
	...
	REVIEW_AI_UNAVAILABLE:
		'ИИ временно недоступен — не удалось подготовить черновик ответа. Повторите через минуту.',
	REVIEW_REPLY_REQUIRED: 'Текст ответа не может быть пустым.',
	REVIEW_PUBLISH_FAILED: 'Канал не принял ответ — попробуйте опубликовать ещё раз позже.',
	// — уведомления / виджет —
