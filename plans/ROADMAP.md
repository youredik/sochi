...
**Track A.bis IN PROGRESS — 4/6 sub-phases pushed origin/main `c9ad1ed` (2026-05-12)**. Hotelier Admin App-Shell Sidebar. Plan canon: [`plans/track-a-bis-canonical.md`](track-a-bis-canonical.md). R1+R2+R3+R4 empirical-verify ≥2026-05-12 — 22+ corrections + per-sub-phase POST-AUDIT C32-C40 (9 new process corrections captured). **7 active sidebar destinations** + dashboard refactored к 4 tactical operator KPIs + Recent activity + Alerts. **Vaul → Base UI 1.4.1 GA Drawer** migration done. **shadcn sidebar primitive** in-repo с 5 local patches D12-D16. **Cumulative delivered: 192 strict tests** across A.bis.0-3 (target was ~90, overdelivered 2.1×). Discovery breakpoint `md:` (768px) domain-specific для front-desk tablets.

**Sub-phase progress (4/6 DONE — pushed origin/main `c9ad1ed`):**
- ✅ **A.bis.0 DONE** (`150f3e5` impl + `4adf37a` fix-up) — Vaul → Base UI Drawer migration. 1382 unit + 43 widget e2e. Senior learning: transparent migration claim требует real-browser e2e proof (JSDOM missed consent-block nested-interactive, Playwright + @axe-core caught it).
- ✅ **A.bis.1 DONE** (`4d6da48`) — shadcn sidebar primitive `ui/sidebar.tsx` (438 LOC direct write, NOT CLI install) + 5 patches D12-D16 + **35 strict tests** (1.75× over plan). New canon: gh API ground-truth для GitHub issue/PR state (C35).
- ✅ **A.bis.2 DONE** (`4934bb5` main + `6a6f60c` fix-up + `88422f7` plan docs) — App-shell integration: SidebarProvider+AdminSidebar+SidebarInset в `_app.tsx` + 7 RBAC-gated sections (sidebar-sections.ts SSOT) + DemoModeBadge (POST-AUDIT C36 backend /me mode enrichment). DELETE 7 mobile-nav files. **67 strict tests** (2.2× over). 1447 frontend + 4078 backend test:fast green. A.bis.2.fix recovered 22/23 e2e regressions + adopted stankoff testing canon (ratchet pre-push gate + global-mocks setupFile). New canon: C37 Layer 4+5 mandatory per sub-phase.
- ✅ **A.bis.3 DONE** (`3dee303` main + `c9ad1ed` fix-up) — Dashboard tiles→4 tactical KPIs (Заезды/В отеле/Открытый баланс/Письма со сбоем) + Recent activity via NEW `/activity/recent` (C36-pattern) + Alerts. **POST-AUDIT C38** (replaced Occupancy/ADR/RevPAR placeholders per `project_dashboard_external.md` canon + Cloudbeds R1 research) + **C39** (biome prop-name collision: id→slug, role→memberRole) + **C40** (data-dashboard-section namespace separation from sidebar's data-section-id). **90 strict + 7 e2e + axe** (6× over plan ~15). Full chromium 130/130 zero regressions.
- 🟡 **A.bis.4 NEXT** — extended axe matrix 12 cells (3 themes × 4 viewports) + visual smoke 4 viewports + forced-colors spec. Plan §7 row 4. Dashboard's basic e2e + axe done at A.bis.3; extended matrix остался scope для A.bis.4.
- ⏳ **A.bis.5** — Closure: ROADMAP A.bis row ✅ + `project_a_bis_done.md` consolidated memory + locked-versions update + dependency freshness `pnpm outdated --recursive` audit + coverage floor bump check + plan §10 «Owner sees 10» prose drift fix (D28 corrected к 7 sections).
...
**Track A7 — M10 (historical pre-flight reference)** Plan canon: [`plans/m10_canonical.md`](m10_canonical.md). Memory: `project_m10_canonical.md`. R1+R2+R3+own-curl-verify ≥2026-05-04 done. **24 decisions D1-D24**: TL polling-not-webhook (PMS reads via /search/v1, never pushes) + lastModification−2d overlap + OAuth Client-Credentials JWT 15min (3rps/15rpm/300rph per-IP) + two-step verify→create Checksum + tlRoomTypeId/tlRatePlanId nullable cols / Yandex.Travel НЕТ direct PMS API → Mock impersonates Bnovo CM passthrough (self-build = breach of YT partner agreement) / Ostrovok ETG single creds 4 brands (RateHawk/ZenHotels/B2B.Ostrovok/Ostrovok) + 5-stage SM + partner_order_id UUID v4 rotation + webhook terminal-only opt-in (polling = source of truth) / CloudEvents 1.0.2 envelope + inbox UNIQUE(source, eventId) + pooled inventory SERIALIZABLE FOR UPDATE + outbox 7-day auto-disable / MSW+PactFlow OSS bi-directional contract testing (NOT classic Pact-JS — broken on Node 24 ARM64) / Booking/Expedia/Airbnb sanctions HARD-DISABLE factory level май 2026 / granular consent 3-checkbox per 1 Sept 2025 152-ФЗ / operator/processor split (TL/Bnovo=processor_with_dpa, YT/Ostrovok=independent_operator) / cross-border-transfer gate deny / МВД миграционный учёт ALWAYS hotel-side / inventoryBuffer + walk-in × OTA SOP / Booking.com B.XML phase-2-ready content-type-agnostic / native node:crypto HMAC. Library canon empirically curl-verified: cloudevents@10.0.0 + @pact-foundation/pact@16.4.0 (TODAY) + zod@4.4.3 (TODAY bump) + fast-xml-parser@5.7.2 + croner@10.0.1. Sub-phase split: A7.1 Foundation (24t) → A7.2 TravelLine (24t) → A7.3 YT (16t) → A7.4 ETG (22t) → A7.5 Bidirectional+RU+demo (16t) = ~102 strict tests target.
...
**Track A4 — M9.widget.6 Embed Web Component** ✅ DONE 2026-05-04 — see `project_m9_widget_6_done.md`. 7 commits + closure pushed origin/main `0f91adb`. **117 strict + 8 e2e tests**. Plan D1-D35 (29 + 6 fresh per-sub-phase R1+R2 corrections).
