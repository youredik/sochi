...
## Infrastructure Phase 16 (parallel to Tracks A.bis/A7) ✅ DONE 2026-05-13

**Branch:** `chore/tsgo-pilot` (NOT main — infra migration, will merge с Track B kickoff).

Bun runtime + tsgo TS 7 emit + Vitest→bun:test migration. Originally landed 2026-05-12 as 6 atomic commits + closure today (2026-05-13) adds 5 more:

- ✅ **Phase 16 closure** (`a605c96`) — 10 carry-forward fails root-cause closed: 5 Sidebar `import.meta.env.DEV` → `process.env.NODE_ENV` (Vite→universal) + 2 RUM `__resetForTesting()` (bun:test module reset) + 1 tabular-nums `Bun.file+import.meta.dir` (Vite glob→Bun-native) + 3 ICS `TZ=Europe/Moscow` (Bun test UTC default → prod parity).
- ✅ **T3 Bun bump** (`61d72e9`) — 1.3.13 → **1.3.14** (released 2026-05-13T04:02 UTC) + workbox-build@7.4.1 direct devDep (pnpm.overrides transitive bug workaround) + drop `--sourcemap` from Dockerfile bun --compile (preventive against 4 OPEN sourcemap PRs filed 2026-05-12).
- ✅ **T1 Stryker drop** (`9144493`) — honest pause; 3 empirical alternatives failed (hughescr-bun-runner Inspector timeout, command-runner concurrency=4 SIGTERM, command-runner concurrency=1 401-flake). Re-enable trigger: stryker-mutator/stryker-js#5424 closes OR frontend test isolation cleanup.
- ✅ **T2 Docker BuildKit cache-mount** (`5a44dc0`) — `pnpm fetch + install --offline` canon; empirical **62× warm rebuild speedup** (1:51 → 1.79s).
- ✅ **T4 Playwright workers parallel** (`fca6f13`) — `workers: process.env.CI ? 2 : 1`, drop hardcoded `--workers=1` from `e2e:smoke`. Cheap-win ~1.8× CI expected; per-worker tenant via `testInfo.workerIndex` deferred (~3-4× canonical).

**Memory canon authoritative**: [[phase-16-closure-done]] (read FIRST), [[bun-test-canons-2026-05-13]], [[round-2-findings-2026-05-13]], [[pnpm-overrides-transitive-bug]].

**Honest half-measures inventory** (next session scope) — 102 act() warnings + 10 401 fetches blocking Stryker re-enable + 9 deprecated transitive subdeps + per-worker tenant isolation refactor + tsgo decorator canary в widget-embed. Tracked in [[phase-16-closure-done]] §«honest half-measures».
