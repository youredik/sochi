...
**Track A5 — M9.widget.7 Perf + a11y CI gate** (pre-flight DONE 2026-05-04). Plan canon: [`plans/m9_widget_7_canonical.md`](m9_widget_7_canonical.md). Memory: `project_m9_widget_7_canonical.md`. R1 broad + R2 adversarial ≥2026-04-15 (today 2026-05-04). **18 decisions D1-D18 + 5 R2 critical corrections**: Lighthouse CI 0.15.1 + LH 12.6.1 (INP NOT lab metric — field-only via web-vitals 5 attribution); LCP gaming defense (`lcp-lazy-loaded: error` + `largest-contentful-paint-element: error` + TBT `aggregationMethod: pessimistic`); `numberOfRuns: 5` median + 100ms tolerance band; **MANDATORY anonymize.ts pipeline** для 152-ФЗ (strips `[name=...]`/`[value=...]`/`aria-label`/`placeholder`/IDs/UA-bucket/IP-truncate before web-vitals attribution → backend RUM); YC Monitoring proprietary HTTP API (NOT OTLP) с reservoir-sample + 10k-metrics/req batch + Cockatiel circuit-breaker; axe-core 4.11.4 EXACT pin + tuple-allowlist (NO blanket disable); size-limit@11.x SPA budgets; Speculation Rules `requires: anonymous-client-ip-when-cross-origin` + `href_matches` own-origin only + `Sec-Purpose: prefetch → 503` cross-origin; `<noscript>` fallback в iframe-html для strict-CSP RU gov sites; forced-colors + Cyrillic visual a11y `toHaveScreenshot()` (Segoe UI Variable ligature drop); CSP `connect-src` exact allowlist + Playwright `--no-network` audit at deploy gate (polyfill.io supply-chain class). Recurring косяки applied UPFRONT в plan §0 (18-row mitigation table). Sub-phase split: **A5.1 foundation DONE `f3bac87` (14 strict tests)** → **A5.2 RUM pipeline DONE 2026-05-04 `05c76b7` (46 strict tests — anonymize 152-ФЗ + web-vitals 5 attribution + YC Monitoring HTTP exporter + circuit-breaker + drop-oldest reservoir)** → **A5.3 axe matrix DONE 2026-05-04 `dd99cca` (55 strict tests — 48 axe scans 4×3×4 + 4 forced-colors visual + 2 noscript Playwright + 1 IF11 unit; D13 noscript fallback + D14 Cyrillic visual smoke + D16 tuple-allowlist no blanket disable)** → **A5.4 Speculation Rules + Sec-Purpose DONE 2026-05-04 (15 strict tests — 10 SP middleware + IF12/IF13 unit + 3 SR-RUM phantom-session filter; D11 anonymous-client-ip + own-origin scope, D12 cross-origin prefetch 503)** → A5 closure (empirical lhci collect + done memory) (~62 strict tests + Lighthouse + axe matrix).
