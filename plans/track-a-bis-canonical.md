...
|---|---|---|---|---|
| **A1.1** | `Cmd+B/Ctrl+B` window-keydown captures even when focus lives in `<input>`/`<textarea>`/`<select>`/contenteditable — `preventDefault()` leaks into form's bold shortcut + sidebar toggles mid-typing | `ui/sidebar.tsx:127` keydown listener | medium | PATCH-D27 input-focus guard — 5 new K5-K9 unit tests |
...
- `apps/frontend/src/components/ui/sidebar.tsx` — PATCH-D27 input-focus guard + PATCH-D15 tightening
