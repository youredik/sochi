...
export const SIDEBAR_SECTIONS = [
	...
	},
	{
		...
		icon: Building2,
		path: '/o/$orgSlug/properties/$propertyId/content',
		permission: { compliance: ['read'] } /* OR amenity:read — runtime check */,
	},
	{
		id: 'guests',
		labelRu: 'Гости',
		...
		permission: { migrationRegistration: ['read'] },
	},
	{
		id: 'channels',
	...
	},
	{
		...
		icon: Bell,
		path: '/o/$orgSlug/admin/notifications',
...
_Заполняется по мере landing каждой sub-phase._
...
### A.bis.5 (Closure) — DONE 2026-05-12 (after senior self-audit `f879d25` + final sweep)

**Senior self-audit phase complete** (user push «сделай сеньер селф аудит» 2026-05-12 после первичного closure docs `aa0ac24` + `f879d25` dep bump). Cross-checked own claims против код / git / plan. Surfaced **3 concrete bugs in my own fix-up commits** + **4 halfmeasures violating `feedback_no_halfway.md`** + **1 real production BUG found by adversarial e2e**.

#### Audit findings + fixes

| #              | Finding                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Where                                     | Status                                                                                                                                                                                                                                                                                               |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A-AUDIT-1**  | PATCH-D27 naming collision — D27 уже занят plan §4 «Top-bar удаляется». Мой rename (D23→D27) создал НОВУЮ collision. Patch-Dxx номера зарезервированы за plan §4 architecture decisions D17-D40. Senior fix: отдельный prefix `PATCH-BH1` (Bug-Hunt-1).                                                                                                                                                                                                                                                 | `ui/sidebar.tsx` + tests + plan canon     | ✅ renamed                                                                                                                                                                                                                                                                                           |
| **A-AUDIT-2**  | Off-by-one в claim «5 commits локально» — actual git log shows 4 (c70c15b/cc90f62/aa0ac24/f879d25)                                                                                                                                                                                                                                                                                                                                                                                                      | Final summary text                        | ✅ corrected to actual count                                                                                                                                                                                                                                                                         |
| **A-AUDIT-3**  | Plan §17 missing dep-bump entry для `f879d25`                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Plan §17 implementation log               | ✅ entry added below                                                                                                                                                                                                                                                                                 |
| **A-AUDIT-4**  | Halfmeasure: Coverage floor check skipped citing test-loop-canon — test-loop-canon про test:serial, не coverage. Real reason: scope-cutting.                                                                                                                                                                                                                                                                                                                                                            | Plan §7 row 5 item (h)                    | ✅ ran `pnpm coverage` — measured 65.69/63.91/61.05/64.48 vs prior floor 60/60/55/60 → bumped к 65/63/60/64 в `vitest.config.ts`                                                                                                                                                                     |
| **A-AUDIT-5**  | Halfmeasure: `pnpm overrides` для serialize-javascript CVE deferred к «security sub-phase post-Track B». Senior мог бы добавить override now while в dep-bump scope.                                                                                                                                                                                                                                                                                                                                    | `package.json` pnpm.overrides             | ✅ added `serialize-javascript: ^7.0.5` — `pnpm audit --prod` теперь «No known vulnerabilities found»; ratchet `audit_high_critical_max` 1→0 в same commit                                                                                                                                           |
| **A-AUDIT-6**  | Halfmeasure: adversarial vectors из initial bug-hunt scope не протестированы (BroadcastChannel multi-tab + multi-org + useCurrentRole undefined + propertyId never-resolves). Honest correction post-audit: useCurrentRole+propertyId УЖЕ покрыты unit-тестами P1/P2 в admin-sidebar.test.tsx (audit overstated gap). Real gaps: BroadcastChannel multi-tab + multi-org.                                                                                                                                | tests/e2e                                 | ✅ new `admin-multi-tab-broadcast.spec.ts` (3 tests: production-flow logout + forward-compat v:999 ignore + cross-tab race). Multi-org honestly deferred (needs separate fresh-tenant signup fixture, out of session scope)                                                                          |
| **A-AUDIT-7**  | Halfmeasure: 10 deprecated subdependencies surfaced by pnpm — defer rationale weak                                                                                                                                                                                                                                                                                                                                                                                                                      | pnpm-lock.yaml warns                      | ⏸ Honestly deferred к dedicated dep-modernization sub-phase. Rationale: leaf transitive deprecation warnings (glob@7.2.3/inflight/rimraf 2&3/source-map-beta/uuid 8&10/node-domexception), NOT CVEs, NOT blocking. Overriding each carries peer-dep break risk; needs dedicated investigation cycle. |
| **🐛 BUG-BH8** | **REAL PRODUCTION BUG** found by `admin-multi-tab-broadcast.spec.ts`: peer-tab `onLogout`/`onOrgChange` handlers в `_app.tsx:77` используют `router.navigate({ to: '/login' })` БЕЗ `reloadDocument: true`. `/login` route's beforeLoad reads cached `sessionQueryOptions` (invalidation async, hasn't completed) → sees still-valid session → bounces peer tab back. Tab A's `useSignOut` уже использует `reloadDocument: true`; peer-tab handler MUST mirror — иначе multi-tab logout silently fails. | `apps/frontend/src/routes/_app.tsx:77-87` | ✅ both handlers (onLogout + onOrgChange) added `reloadDocument: true`. Test now passes 3/3.                                                                                                                                                                                                         |

#### Final dep-bump + audit verification (after all self-audit fixes)

- typecheck ✓ EXIT=0 across 4 workspaces
- 5 pre-commit gates ✓ sherif/biome/depcruise/knip/typecheck
- ratchet ✓ depcruise=0 knip=0 **audit_high=0** ts_err=0 biome_err=0 weak_assertions=234 multi_biome_ignore=0 (tightened 7→6→1→0 across 3 commits)
- `pnpm test:fast` ✓ **4185 passed / 0 failed**
- `pnpm coverage` ✓ measured 65.69 lines / 63.91 branches / 61.05 functions / 64.48 statements (floor ratcheted 60/60/55/60 → 65/63/60/64)
- Playwright chromium ✓ **160/160** (158 baseline + 2 new multi-tab broadcast — production-flow logout + v:999 forward-compat-ignore; the 3rd test from initial draft was merged into the production-flow scenario)
- Playwright smoke ✓ **68/68** (no change vs prior baseline)
- Cumulative ✓ **228/228** e2e green

#### Honest carry-forward (NOT в A.bis scope, документировано)

- **Multi-org switch sidebar reactivity** e2e — needs separate fresh-tenant signup fixture. Defer к multi-org feature sub-phase (whenever org-switcher UI gets real polish).
- **10 deprecated subdeps** batch — dedicated dep-modernization sub-phase.
- **Per-objectType activity RBAC SQL-level filter** — current post-query filter is defence-in-depth; SQL-level requires YDB WHERE-IN unnest. Defer к activity-domain optimization.
- **234 weak_assertions** baseline opportunity — strict-test cleanup sub-phase.
...
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- | -------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| **A1.1**                 | `Cmd+B/Ctrl+B` window-keydown captures even when focus lives in `<input>`/`<textarea>`/`<select>`/contenteditable — `preventDefault()` leaks into form's bold shortcut + sidebar toggles mid-typing | `ui/sidebar.tsx:127` keydown listener    | medium         | PATCH-BH1 input-focus guard — 5 new K5-K9 unit tests                                                                          |
...
- `apps/frontend/src/components/ui/sidebar.tsx` — PATCH-BH1 input-focus guard + PATCH-D15 tightening
