...
- `src/index.css` — заменить shadcn primary OKLCH на Sochi-blue (start values, **post-empirical adjust**):
  ...
  .dark {
  	--primary: oklch(0.7 0.16 240); /* lighter Sochi-blue для dark — START */
  	--primary-foreground: oklch(0.205 0 0); /* dark-on-light-blue */
  }
  @media (prefers-contrast: more) {
  	:root {
  		--primary: oklch(0.45 0.2 240);
  	} /* deeper для AAA — START */
  	.dark {
  		--primary: oklch(0.78 0.18 240);
  	} /* lighter для AAA — START */
  }
  ```
...
- `src/index.css` — добавить 4-level shadow tokens в `@theme inline`:
  ...
  @theme inline {
  	...
  	--shadow-drawer: 0 -8px 24px -4px oklch(0 0 0 / 0.1);
  ...
  .dark {
     ...
     * Inset highlight + soft shadow создают depth без heavy drop-shadow. Layered surfaces canon 2026. */
  	--shadow-card: 0 1px 2px 0 oklch(0 0 0 / 0.2), inset 0 1px 0 0 oklch(1 0 0 / 0.03);
  	--shadow-popover: 0 4px 12px -2px oklch(0 0 0 / 0.4), inset 0 1px 0 0 oklch(1 0 0 / 0.05);
  	--shadow-drawer: 0 -8px 24px -4px oklch(0 0 0 / 0.5), inset 0 1px 0 0 oklch(1 0 0 / 0.06);
  	--shadow-dialog: 0 16px 48px -8px oklch(0 0 0 / 0.6), inset 0 1px 0 0 oklch(1 0 0 / 0.08);
  }
  ```
- Применить:
  ...
  function Skeleton({ className, ...props }) {
  	return (
  		<div
  			role="status"
  			aria-busy="true"
  			className={cn('skeleton-shimmer motion-reduce:skeleton-pulse rounded-md', className)}
  			{...props}
  		>
...
- min-h-screen → min-h-svh codemod (3 files: index.css, \_\_root.tsx, \_app.tsx) — Baseline Widely Available June 2025
...
re-evaluation. Integration: `chessboard.tsx`containerRef + ternary`windowDaysPref === 'fit' ? fitDays : windowDaysPref`
...
- **YDB migration `0043_passkey_better_auth.sql`:**
  - `passkey` table (id typeid pk\_\*, name nullable, publicKey, userId,
